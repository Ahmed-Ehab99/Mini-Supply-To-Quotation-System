...
function SupplierOfferIntakePage() {
  ...
  const form = useForm<SupplierOfferFormValues>({
    ...
    defaultValues: {
      ...
      valid_until: "", status: "active", notes: "",
    },
  });
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              <div className="space-y-2">
                ...
                <ComboboxField
                  ...
                  onChange={(v) => form.setValue("material_id", v, { shouldValidate: true })}
                  placeholder="Select a material"
                />
                {form.formState.errors.material_id && (
                  <p className="text-xs text-destructive">{form.formState.errors.material_id.message}</p>
                )}
              </div>
              <div className="space-y-2">
                ...
                <ComboboxField
                  options={(suppliers ?? []).map((s) => ({
                    label: s.name, value: s.id, hint: s.country ?? undefined,
                  }))}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button type="button" variant="outline" onClick={() => navigate({ to: "/offers" })}>Cancel</Button>
                <Button type="submit" disabled={create.isPending}>Submit Offer</Button>
              </div>
            </form>
          </CardContent>
        </Card>
        ...
        <Card className="h-fit">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                <p className="text-xs uppercase tracking-wide text-muted-foreground">Valid from</p>
